...
if (isset($_GET['catname'])) {
  ...
  // Check if age parameter is set
  if (isset($_GET['age'])) {
    $age = $_GET['age'];
    $sql = "SELECT * FROM post
            JOIN category ON post.category = category.category_name
            WHERE category.category_name = '$catname' AND category.age = '$age'";
  } else {
    $sql = "SELECT * FROM post
            JOIN category ON post.category = category.category_name
  ...
  $category_name = mysqli_fetch_assoc(mysqli_query($conn, "SELECT category_name FROM category WHERE category_name='$catname'"))['category_name'];
  if(mysqli_num_rows($result) == 0) {
    echo "<div style='display: flex; align-items: center; justify-content: center; height: 100vh;'>
          <div style='text-align: center;'>
            <h2>No posts available for category '$catname'</h2>
            <button style='background-color: #4CAF50; color: white; padding: 10px 20px; border: none; border-radius: 4px; cursor: pointer;' onclick='window.location.href=\"index.php\"'>Go Back</button>
          </div>
        </div>";
    exit();
  }
